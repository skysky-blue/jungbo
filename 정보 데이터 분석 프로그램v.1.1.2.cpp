...
double incline(){ //선형회귀 기울기 구해주는 변수 
	...
	all[3][1]=0,all[3][2]; //all[3][1]에는 선형회기에서 기울기를 구할때 필요한 시그마의 분자 값을 받아준다. all[3][2]에는 분모 값을 받아준다. 
	for (int i =0; i<term;i++){
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][2]+= sqrt(((i+1)*wid)+9-Ex[2][1]); //분모 값을 시그마 해준다. 
	}
