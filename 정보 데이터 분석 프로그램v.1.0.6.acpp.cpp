...
struct stock{ //주식 정보 받아오는 구조체 구조 선언 
	...
	char data[20]; //데이터 한 줄을 받음 
	char *dangi; //당기순이익 정보 받음 
	char *yeop; // 엽업 이익 정보 받음
	char *stck; //평균 주가 정보 얻음 
};
...
//		}
		...
		sscanf(line, "%lf, %s",&comp[index].date,comp[index].data); // 년도_분기, 당기순이익, 영업이익, 평균 주가 입력 받기 
		index++;
	}
	 
	for (int i = 0; i<(index-2); i++){
		char *ptr = strtok(comp[i].data,","); //data 값을 ,단위로 자른다.
		comp[i].dangi=ptr;
		ptr =strtok(NULL, ",");
		comp[i].yeop=ptr;
		ptr =strtok(NULL, ",");
