...
int wi =0,dangiwi=0,stockwi=0; //x축의 너비, 당기순이익 너비, 주가 너비를 저장해주는 변수 
...
double max_a_y_1=0, min_a_y_1=0 ,max_b_y_1=0, min_b_y_1=0, max_st_1=0, min_st_1=0; //max함수,min함수 만들고 나서 적기 
...
//		printf("%s",filename[company]);
		...
		if (excel_true !=-1){
			...
			axis(s_com,s_comQ,f_com,f_comQ,max_a_y_1,min_a_y_1,max_st_1,min_st_1,max_b_y_1,min_b_y_1); //나중에 여기 당기순이익, 주가, 영업이익 입력해주기 wid는 x축 상에서 데이터 값의 너비, 여기다 +9를 해주면 넣어야할 데이터의 x좌표가 나온다. 
			gotoxy(20,10);
//			printf("%d",wid);
			gotoxy(1,1);
