...
//		company_position=select();
		...
		axis(s_com,s_comQ,f_com,f_comQ,720,120,500,250);
		while (true){
			int i=1;
		}
		restart=1;
...
void draw_st (int max_st, int min_st,int max_a_y, int min_a_y){ //y축 그리기   
	int k = 0; // 단위 표시가 몇 번 반복 되었는지 확인하는 변수 
	int n=0; //n은 y_a_dif = 1이라면 96 정도를 0이라면 144정도를 입력해주면 된다.
	if (y_a_dif(max_st,min_st)) n=144;
	else n=96; 
	for (int i = 3; i <= (n+3); i++){
		gotoxy(99,i);
		printf("|");
...
//	printf("%d",width);
	...
	for (int i = 0; i<=(term); i++){
		if (i<=(term-1)){
			...
			print_x(0,(i*width)+10,p,width,start_company,start_company_Q,finish_company,finish_company_Q);
...
void print_x(int k, int x, int y, int width,int sc ,int scq,int fc,int fcq){ //x축 ---| 한 번 그리는 함수 
	...
	printf("|");
	print_com(k,x+width-1,y+1,sc+((scq+coms)/4),(scq+coms)%4); // | 밑에 년도, 분기를 한 글자씩 출력해준다. 
	}
}
void print_com(int k, int x, int y,int sc ,int scq){ //x축에 년도, 분기 적기
	if (k==1){
