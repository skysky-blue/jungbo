...
int lim_range_company[100]={0,2020,2020,2020,2020,2020,2020};  //기업의 데이터 사용 가능 최대 범위에서 년도 표시 배열
...
int all[100][100]; //변수 값 모두 받아주는 배열로 평균을 구할 때 사용한다.  
double Ex[50][50]; //[1]에서 [1]에는 당기순이익의 평균을 [2]에는 영업이익의 평균을 [3]에는 주가의 평균을 넣을 배열이다. 
// [2]에서 [1]은 x의 평균을 넣는다. 
...
int st_gr(int max_st, int min_st, int wid, int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q); //주식 그래프 그리기 
double Exd(int wid,int dangiwi ,int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q); //평균을 구해주는 함수  
double incline(); //선형회귀 기울기 구해주는 변수 
double y_jeol(int inclines); //y절편 구해주는 함수 
double linear(int wid,int dangiwi ,int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q); //선형회귀 그래프 그리는 함수 
int exit_linear(); //그래프 출력한 뒤 처음 메뉴화면으로 돌아가도록 해주는 함수
...
int main(){
	...
	while(1){
		...
		if (excel_true !=-1){
			...
			linear( wi, dangiwi , stockwi,  s_com,s_comQ,f_com,f_comQ); 
			exit_linear();
		}
	restart=1;
}
...
int st_gr(int max_st, int min_st, int wid, int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q){
	int po_ind=(start_company-2010)*4+start_company_Q; //시작하는 데이터 범위가 comp구조체의 몇번째 index에 위치해있는지 확인해주는 변수이다.
	float st_y=0; //주식의 y좌표 받아주는 변수 
	textc(10);//텍스트 색 초록색으로 바꾸기  
	for (int i =1; i <=term; i++){
		st_y=6+(36-(comp[i+po_ind-1].stck/stockwi))*4;
		gotoxy((i*wid)+9,st_y);
		printf("* ");
	}
	textc(15); //텍스트 색 흰색으로 다시 초기화 
}

double Exd(int wid,int dangiwi ,int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q){ //평균을 구해주는 함수  
	int po_ind=(start_company-2010)*4+start_company_Q; //시작하는 데이터 범위가 comp구조체의 몇번째 index에 위치해있는지 확인해주는 변수이다.
	for (int i=0;i<term;i++){
		all[1][1]+=6+(36-(comp[i+po_ind].dangi/dangiwi))*4; //all[1][1] 에는 당기순이익의 총합을 넣어준다.
		all[2][1]+=6+(36-(comp[i+po_ind].yeop/dangiwi))*4; //all[2][1] 에는 영업 이익의 총합을 넣어준다.
		all[3][1]+=6+(36-(comp[i+po_ind].stck/stockwi))*4; //all[3][1] 에는 주가의 총합을 넣어준다.
		all[1][2]+= ((i+1)*wid)+9; //all[1][2]에는 x값의 총합을 넣어준다. 
	}
	Ex[1][1]=all[1][1]/term;// 당기순이익 평균 대입 
	Ex[1][2]=all[2][1]/term;// 영업이익 평균 대입 
	Ex[1][3]=all[3][1]/term;// 주가 평균 대입 
	Ex[1][4]=(Ex[1][1]+Ex[1][2]+Ex[1][3])/3;//모든 값의 평균을 대입 
	Ex[2][1]=all[1][2]/term;// x값 평균 대입 
	return 0;
}

double incline(int wid,int dangiwi ,int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q){ //선형회귀 기울기 구해주는 변수 
	double inclines=0; // 기울기 값 받는 변수  
	int po_ind=(start_company-2010)*4+start_company_Q; //시작하는 데이터 범위가 comp구조체의 몇번째 index에 위치해있는지 확인해주는 변수이다.

	all[3][1]=0,all[3][2]; //all[3][1]에는 선형회귀에서 기울기를 구할때 필요한 시그마의 분자 값을 받아준다. all[3][2]에는 분모 값을 받아준다. 
	for (int i =0; i<term;i++){
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][1]+= (6+(36-(comp[i+po_ind].dangi/dangiwi))*4-Ex[1][4])*(((i+1)*wid+9)-Ex[2][1]);
		all[3][2]+= pow(((i+1)*wid)+9-Ex[2][1], 2); //분모 값을 시그마 해준다. 
	}
	inclines=all[3][1]/all[3][2]; //기울기 값 받아오기
	return inclines; 
}

double y_jeol(int inclines){ //y절편 구해주는 함수 
	int y_jeol = 0;
	y_jeol = Ex[1][4]-inclines*Ex[2][1]; //y절편 변수에 값 대입
	return y_jeol; //y절편 리턴 
} 

double linear(int wid,int dangiwi ,int stockwi, int start_company, int start_company_Q, int finish_company, int finish_company_Q){ //선형회귀 그래프 그리는 함수 
	double inclines =0,y_jeon=0; //기울기,y절편 값 받는 변수
