...
const char *filename[100]={0,"samsung.csv","apple.csv","amazon.csv","microsoft.csv","tsmc.csv","sk hynix.csv"}; //파일 불러올 때 이름 배열 선언 filename[company]하면 그 회사의 파일 이름이 나옴 
...
int excel(); //excel 데이터 불러와주는 함수  


struct stock{ //주식 정보 받아오는 구조체 구조 선언 
	char date[10]; //년도_분기 정보 받음 
	float dangi; //당기순이익 정보 받음 
	float yeop; // 엽업 이익 정보 받음
	float stck; //평균 주가 정보 얻음 
};
...
int graph(int wid, int dangiwid, int stockwid); //그래프에 데이터 값을 *로 표시해주는 함수 
...
int main(){
	...
	int wid =0,dangiwid=0,stockwid=0; //x축의 너비, 당기순이익 너비, 주가 너비를 저장해주는 변수 
	while(1){
		coms=0; 
		start();
		if (position == 1){
		...
		if (excel_true !=-1){
			...
			//dangiwid, stockwid, wid = axis(s_com,s_comQ,f_com,f_comQ,720,300,500,250,700,280); //나중에 여기 당기순이익, 주가, 영업이익 입력해주기 wid는 x축 상에서 데이터 값의 너비, 여기다 +9를 해주면 넣어야할 데이터의 x좌표가 나온다. 
			//gotoxy(1,1);
...
int excel(){ //excel 데이터 불러와주는 함수 
	...
	int index = 0; //구조체의 배열에서 회사 번호마다 하나씩 넣기 
...
//		}
		...
		sscanf(line, "%6s,%5f,%5f,%5f",comp[index].date,&comp[index].dangi,&comp[index].yeop,&comp[index].stck); // 년도_분기, 당기순이익, 영업이익, 평균 주가 입력 받기 
		index++;
	}
	for (int i = 0; i<(index-2); i++){
		printf("%6s:date,%5f:dangi,%5f:yeop,%5f:stck\n",comp[i].date,comp[i].dangi,comp[i].yeop,comp[i].stck);
	}
} 

int dan_gr(int wid, int dangiwid,int start_company ,int start_company_Q,int finish_company,int finish_company_Q){ //당기 순이익 그래프 그려주는 함수 
	//comp.dangi[company]; 
}


int graph(int wid, int dangiwid, int stockwid,int start_company ,int start_company_Q,int finish_company,int finish_company_Q ){ //그래프에 데이터 값을 *로 표시해주는 함수 
