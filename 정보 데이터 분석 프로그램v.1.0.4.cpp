...
void draw_y ( int max_a_y, int min_a_y, int max_b_y, int min_b_y) ;  //y축에 단위 색깔바꾸고 입력해주는 함수
...
struct stock{ //주식 정보 받아오는 구조체 구조 선언 
	...
	int dangi; //당기순이익 정보 받음 
	int yeop; // 엽업 이익 정보 받음
...
int main(){
	...
	while(1){
		...
		if (excel_true !=-1){
			...
			wid = axis(s_com,s_comQ,f_com,f_comQ,720,300,500,250,700,280); //나중에 여기 당기순이익, 주가, 영업이익 입력해주기 
			gotoxy(1,1);
...
void draw_y ( int max_a_y, int min_a_y, int max_b_y, int min_b_y){ //y축 그리기   
